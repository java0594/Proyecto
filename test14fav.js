...
async function obtenerEvolucionAvanzada(speciesUrl) {
    ...
    async function procesar(node) {
        ...
        const sprite = info.ok ? info.data.sprite : "";
...
// -----------------------------------------------------
// OBTENER SPRITE SIMPLE DE POKÉMON (para habilidades)
// -----------------------------------------------------
async function obtenerPokemonSimple(url) {
    const parts = url.split("/").filter(Boolean);
    const nombre = parts[parts.length - 2]; // penúltimo elemento es el nombre
...
function guardarFavorito(pokemon) {
    ...
    if (existe) {
        ...
        favs.push({ id: pokemon.id, name: pokemon.name, sprite: pokemon.sprite });
...
async function buscar() {
    ...
    if (tipo === "POKÉMON") {
        await mostrarDatosInterna(entrada);
    } else if (tipo === "HABILIDAD") {
        await mostrarHabilidadInterna(entrada);
    } else {
...
function mostrarFavoritos() {
    ...
    if (favs.length === 0) {
        result.innerHTML = "<p>No hay Pokémon favoritos aún.</p>";
        return;
    }
...
document.addEventListener("DOMContentLoaded", ()=> {
    ...
    if (document.getElementById("result") && window.location.pathname.includes("favoritos.html")) {
        mostrarFavoritos();
    }
});
