// =====================================================
// test31.js  (Base test27 + VS aislado listo para completar)
// =====================================================
...
// =====================================================
// ⚔️ VS MODULE (AISLADO, LISTO PARA COMPLETAR)
// =====================================================
let vsLeft = null;
let vsRight = null;
...
function vsRenderPreview() {
    ...
    <div class="card">
      ...
      ${safeArr(r.data.types).map(t => `<span class="label">${t}</span>`).join("")}
      <div style="margin-top:10px;"></div>
      <button class="btn-white" type="button">⭐ Favorito</button>
    </div>
  `;

    // Nota: el botón favorito del VS por ahora NO hace nada (lo conectamos luego si quieres)
    cont.innerHTML = `
...
function vsRenderStatus() {
    ...
    // “Ganchos” listos para que luego pintemos:
    // - ventajas de tipo
    // - comparación de stats con barras
    // - cálculo detallado del puntaje
    out.innerHTML = `
    <div class="card" style="max-width:900px;">
      <h2>⚔️ RESULTADO DE LA BATALLA</h2>
      <p>Selecciona 2 Pokémon y presiona <strong>BATALLAR</strong>.</p>

      <div id="vs-analysis" style="margin-top:20px;">
        <div id="vs-type-adv" class="card" style="background:#e0e0e0;">
          <h3>⚡ Ventajas de Tipo</h3>
          <p>(Pendiente: multiplicadores x0.5 / x2.0 como en tu imagen)</p>
        </div>

        <div id="vs-stats-compare" class="card" style="background:#e0e0e0;">
          <h3>📊 Comparación de Stats</h3>
          <p>(Pendiente: barras comparativas HP/ATK/DEF/SP.ATK/SP.DEF/SPD)</p>
        </div>

        <div id="vs-score-calc" class="card" style="background:#e0e0e0;">
          <h3>🧮 Cálculo del Puntaje</h3>
          <p>(Pendiente: totalStats, multiplicadorTipo, puntajeFinal)</p>
        </div>
      </div>
    </div>
  `;
...
function vsBatallar() {
    ...
    // Por ahora: ganador SOLO por suma base (sin multiplicadores).
    // Luego lo extendemos al modelo de tu imagen.
    let winner = "EMPATE";
    if (total1 > total2) winner = p1.name.toUpperCase();
    if (total2 > total1) winner = p2.name.toUpperCase();
    ...
    <div class="card" style="max-width:900px;">
      ...
      <div style="display:flex; gap:20px; justify-content:center; flex-wrap:wrap; margin-top:20px;">
        <div class="card" style="background:${total1 > total2 ? "#7CFF7C" : total1 < total2 ? "#e0e0e0" : "#ffe68a"}; min-width:240px;">
          <img class="sprite" src="${p1.sprite}" alt="${p1.name}">
          <h3>#${p1.id} ${p1.name.toUpperCase()}</h3>
          ${safeArr(p1.types).map(t => `<span class="label">${t}</span>`).join("")}
          <p style="margin-top:10px;"><strong>${total1}</strong> pts (base)</p>
        </div>

        <div class="card" style="background:${total2 > total1 ? "#7CFF7C" : total2 < total1 ? "#e0e0e0" : "#ffe68a"}; min-width:240px;">
          <img class="sprite" src="${p2.sprite}" alt="${p2.name}">
          <h3>#${p2.id} ${p2.name.toUpperCase()}</h3>
          ${safeArr(p2.types).map(t => `<span class="label">${t}</span>`).join("")}
          <p style="margin-top:10px;"><strong>${total2}</strong> pts (base)</p>
        </div>
      </div>
      ...
      <div id="vs-analysis" style="margin-top:20px;">
        <div id="vs-type-adv" class="card" style="background:#e0e0e0;">
          <h3>⚡ Ventajas de Tipo</h3>
          <p>(Pendiente: multiplicadores x0.5/x2.0 y texto como en tu imagen)</p>
        </div>

        <div id="vs-stats-compare" class="card" style="background:#e0e0e0;">
          <h3>📊 Comparación de Stats</h3>
          <p>(Pendiente: barras comparativas por stat)</p>
        </div>

        <div id="vs-score-calc" class="card" style="background:#e0e0e0;">
          <h3>🧮 Cálculo del Puntaje</h3>
          <p><strong>Stats Base Total:</strong> ${p1.name}: ${total1} | ${p2.name}: ${total2}</p>
          <p><strong>Multiplicador de Tipo:</strong> (pendiente)</p>
          <p><strong>Puntaje Final:</strong> (pendiente)</p>
        </div>
      </div>
    </div>
  `;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // VS (solo si existe el DOM del VS)
    if (vsHasDom()) {
