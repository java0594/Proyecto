...
async function mostrarPokemonInterno(entrada) {
    ...
    let evolucion = null;
    try { evolucion = await obtenerEvolucionAvanzada(pokemon.speciesUrl); } catch { }
...
function vsBatallar() {
    ...
    else if (total2 > total1) winner = p2.name.toUpperCase();

    const rows = vsCompareStats(p1, p2);

    const highlightCard = (isWinner, isLoser) => {
        if (winner === "EMPATE") return "#ffe68a";       // empate
        if (isWinner) return "#7CFF7C";                  // ganador
        if (isLoser) return "#e0e0e0";                   // perdedor
        return "#fff";
    };

    const leftIsWinner = winner !== "EMPATE" && winner === p1.name.toUpperCase();
    const rightIsWinner = winner !== "EMPATE" && winner === p2.name.toUpperCase();
    ...
    <div class="card" style="max-width:900px;">
      ...
      <div style="display:flex; gap:20px; justify-content:center; flex-wrap:wrap; margin-top:20px;">
        <div class="card" style="background:${highlightCard(leftIsWinner, rightIsWinner)}; min-width:260px;">
        ...
        <div class="card" style="background:${highlightCard(rightIsWinner, leftIsWinner)}; min-width:260px;">
      ...
      <div class="card" style="background:#e0e0e0; margin-top:20px;">
        <h3>📊 Comparación de Stats</h3>

        ${rows.map(r => {
            const leftColor = r.winner === "left" ? "green" : (r.winner === "tie" ? "#b36b00" : "");
            const rightColor = r.winner === "right" ? "green" : (r.winner === "tie" ? "#b36b00" : "");
            ...
            return `
              <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin:8px 0;">
                <span style="width:90px; font-weight:bold;">${label}</span>
                <span style="width:60px; text-align:right; font-weight:bold; color:${leftColor};">${r.left}</span>
                <span style="width:30px; text-align:center;">VS</span>
                <span style="width:60px; text-align:left; font-weight:bold; color:${rightColor};">${r.right}</span>
              </div>
            `;
        }).join("")}
      </div>
      ...
      <div class="card" style="background:#e0e0e0; margin-top:20px;">
        <h3>🧮 Cálculo</h3>
        <p><strong>Stats Base Total:</strong> ${p1.name.toUpperCase()} = ${total1} | ${p2.name.toUpperCase()} = ${total2}</p>
        <p><strong>Resultado:</strong> ${winner === "EMPATE" ? "EMPATE" : "GANADOR " + winner}</p>
      </div>
    </div>
  `;
}
